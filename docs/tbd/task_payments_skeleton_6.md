...
- `app/api/payments/[provider]/callback`: проверка подписи → идемпотентность по `orderReference` (уникальный индекс) → `paid` → создание/линковка Users (опирается на `src/lib/auth/` из [task_real_auth_14](../kanban/todo/task_real_auth_14.md)) → письмо-заглушка (лог; Resend — отдельно).
